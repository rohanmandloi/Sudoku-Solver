...
while test != 4:
	...
	if lines is not None:
		...
		for i in l:
			...
			x2 = int(x0 - 1000*(-b))
			y2 = int(y0 - 1000*(a))
			line_detection = cv2.line(imgCopy,(x1,y1),(x2,y2),(0,0,255),2)
			# print x1,y1,x2,y2
			...
			if abs(y1) != 1000:
					l1.append([0,y1,0])
		# This will show you image with all the lines we got from hough lines
		...
		if len(points) > 99:
			...
			while i < len(points)-10:
				...
				if points[i+10][0] - points[i][0] <= 10:
					...
					cv2.circle(imgCopy, (points[i][0],points[i][1]), 5, (255,0,0), 3)
					flag=0
				else:
					final_points.append(points[i])
					cv2.circle(imgCopy, (points[i][0],points[i][1]), 5, (255,0,0), 3)
			...
			if len(final_points) != 100:
				for i in range(len(points)-10, len(points)):
					final_points.append(points[i])
					cv2.circle(imgCopy, (points[i][0],points[i][1]), 5, (255,0,0), 3)
			# This will show you image with all the intersection we got
			cv2.imshow('Intersection', imgCopy)
			cv2.waitKey(0)
